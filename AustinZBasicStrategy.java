 ...
 */
public class AustinZBasicStrategy implements Strategy {
    ...
    private int roomDistanceChromosome;
    private int cardDistanceChromosome;
    private int suggestedRoomChromosome;
    private int startObservationChromosome;
    private int guessThreshold;


    private AustinZExploiter exploiter;

    private AustinZDijkstraPathfinding pathfinder;

    public void setRoomCoeff(int roomCoeff) {
        this.roomDistanceChromosome = roomCoeff;
    }

    public AustinZBasicStrategy(int[] chromosomes) {
        roomDistanceChromosome = chromosomes[0];
        cardDistanceChromosome = chromosomes[1];
        suggestedRoomChromosome = chromosomes[2];
        startObservationChromosome = chromosomes[3];
        guessThreshold = chromosomes[4];
    }

    public AustinZBasicStrategy(int roomDistanceChromosome, int cardDistanceChromosome
                                , int suggestedRoomChromosome, int startObservationChromosome, int guessThreshold) {
        this.roomDistanceChromosome = roomDistanceChromosome;
        this.cardDistanceChromosome = cardDistanceChromosome;
        this.suggestedRoomChromosome = suggestedRoomChromosome;
        this.startObservationChromosome = startObservationChromosome;
        this.guessThreshold = guessThreshold;
        exploiter = new AustinZExploiter(this.guessThreshold);
    }

    public AustinZBasicStrategy() {
        roomDistanceChromosome = 10;
        cardDistanceChromosome = 1;
        suggestedRoomChromosome = 0;
        startObservationChromosome = 0;
        guessThreshold = Integer.MAX_VALUE;
    }
    ...
    public int getActionType(boolean canStay, boolean canUseSecretPassage) {
        ...
        if (exploiter.validSolution(fullDeck, cardsPlayersHave) != null) {
            System.out.println(exploiter.validSolution(fullDeck, cardsPlayersHave).toString());
            currentBestSuggestion = exploiter.validSolution(fullDeck, cardsPlayersHave);
            exploiting = true;
    ...
    public void suggestionMade(int suggester, Suggestion suggestion, int revealer) {
        if (game.getNumTurns() >= startObservationChromosome) {
            exploiter.update(suggestion);
        }
    ...
    private Suggestion bestSuggestion() {
        ...
        */
        ...
        for (int i = 0; i < 9; i++) {
            ...
            scoresForRooms[i] = roomDistanceChromosome * distanceToCards[i] -
                    cardDistanceChromosome * distanceToRooms[i] +
                    suggestedRoomChromosome * exploiter.getRoomsSuggested().get(i);
        }
